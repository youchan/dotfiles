" 他のプラグインはここに追加
call minpac#add('JuliaEditorSupport/julia-vim')
call minpac#add('LeafCage/yankround.vim')
call minpac#add('MetalPhaeton/easybracket-vim')
call minpac#add('Shougo/context_filetype.vim')
call minpac#add('Shougo/ddc.vim')
call minpac#add('Shougo/ddu.vim')
call minpac#add('Shougo/ddu-filter-matcher_substring')
call minpac#add('Shougo/ddu-source-file_rec')
call minpac#add('Shougo/ddu-ui-ff')
call minpac#add('Shougo/ddu-kind-file')
call minpac#add('Shougo/ddu-filter-matcher_substring')
call minpac#add('kuuote/ddu-source-git_status')
call minpac#add('k-ota106/ddu-source-gg')
call minpac#add('airblade/vim-gitgutter')
call minpac#add('andymass/vim-matchup')
call minpac#add('chrisbra/Colorizer')
call minpac#add('chrisbra/Recover.vim')
" call minpac#add('cohama/lexima.vim') " lexima は
call minpac#add('dhruvasagar/vim-table-mode')
" call minpac#add('elixir-editors/vim-elixir')
call minpac#add('fatih/vim-go')
call minpac#add('haya14busa/incsearch.vim')
call minpac#add('jacoborus/tender.vim')
call minpac#add('kana/vim-textobj-indent')
call minpac#add('kana/vim-textobj-user')
call minpac#add('kannokanno/previm')
call minpac#add('kmnk/vim-unite-giti')
" call minpac#add('lambdalisue/gina.vim')
call minpac#add('lambdalisue/vim-gin')
call minpac#add('lambdalisue/vim-fern')
" call minpac#add('leafgarland/typescript-vim')
call minpac#add('posva/vim-vue')
call minpac#add('prabirshrestha/asyncomplete.vim')
call minpac#add('prabirshrestha/asyncomplete-lsp.vim')
call minpac#add('prabirshrestha/vim-lsp')
call minpac#add('mattn/vim-lsp-settings')
call minpac#add('mattn/vim-lsp-icons')
call minpac#add('mattn/emmet-vim')
call minpac#add('hrsh7th/vim-vsnip')
call minpac#add('hrsh7th/vim-vsnip-integ')
call minpac#add('rhysd/vim-gfm-syntax')
call minpac#add('roxma/nvim-yarp')
call minpac#add('roxma/vim-hug-neovim-rpc')
call minpac#add('scrooloose/nerdcommenter')
call minpac#add('sgur/vim-textobj-parameter')
call minpac#add('simeji/winresizer')
call minpac#add('slashmili/alchemist.vim')
call minpac#add('szw/vim-tags')
" call minpac#add('tbodt/deoplete-tabnine')
call minpac#add('thinca/vim-quickrun')
call minpac#add('thinca/vim-ref')
call minpac#add('tpope/vim-afterimage')
call minpac#add('tpope/vim-endwise')
call minpac#add('tpope/vim-repeat')
call minpac#add('tpope/vim-surround')
call minpac#add('vim-skk/eskk.vim')
call minpac#add('tyru/open-browser.vim')
call minpac#add('tyru/skkdict.vim')
call minpac#add('vim-denops/denops.vim')
call minpac#add('vim-jp/syntax-vim-ex')
call minpac#add('vim-jp/vital.vim')
call minpac#add('vim-scripts/ruby-matchit')
call minpac#add('jlcrochet/vim-rbs')
call minpac#add('cespare/vim-toml')
call minpac#add('vim-scripts/vim-auto-save')
call minpac#add('wakatime/vim-wakatime')
call minpac#add('yukimura1227/vim-yazi')
